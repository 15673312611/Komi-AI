@echo off
chcp 65001 > nul
title ChatterMate Dev Stack (Go Backend :8001 + Frontend :3001)

echo ========================================================
echo        ChatterMate Dev Stack
echo ========================================================
echo.
...
if errorlevel 1 (
    if exist "%~dp0pgsql\bin\pg_ctl.exe" (
        ...
        docker start chattermate-db-1 >nul 2>&1
    )
)
...
cd /d "%~dp0backend-go"
go build -o chattermate-server.next.exe ./cmd/server
if errorlevel 1 (
    echo   - Go build failed; services were not started
    del /q chattermate-server.next.exe >nul 2>&1
    exit /b 1
)
move /y chattermate-server.next.exe chattermate-server.exe >nul
cd /d "%~dp0"
...
start /b "" "%~dp0backend-go\chattermate-server.exe"
